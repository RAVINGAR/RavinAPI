...
public class CancelInputAction extends Action {
    private final String menu;

    public CancelInputAction(final String pointer, final String menu) {
        super(pointer);
        this.menu = menu;
    }
    ...
    public void performAction(final BaseGui gui, Player performer) {
        gui.findComponent(Component.MENU, menu).map((menu) -> menu.findComponent(Component.INPUT_COMPONENT, pointer)).ifPresentOrElse((opt) -> {
            opt.ifPresentOrElse((input) -> input.cancelResponse(gui, performer), () -> GuiProvider.log(Level.WARNING, "Could not find input component called " + pointer));
